const handler = async (m, {conn}) => {
  ...
  conn.sendMessage(m.chat, {audio: {url: vn}, ptt: true, mimetype: 'audio/mpeg', fileName: `putos.mp3`}, {quoted: m});
};
handler.customPrefix = /putos|putoz|reputos/
handler.command = /^(putos|putitos|putos?$)/
export default handler;

/*
let handler = async (m, { conn }) => {
if (!db.data.chats[m.chat].audios && m.isGroup) throw 0
let vn = './media/putos.mp3'
conn.sendPresenceUpdate('recording', m.chat)
